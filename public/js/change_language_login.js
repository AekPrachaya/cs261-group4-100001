...
function changeLoginLang(lang = language.en) {
	//เปลี่ยนภาษาหน้า login
...
function changeSidebarLang(lang = language.en) {
	//เปลี่ยนภาษาหน้า sidebar
...
function changeRequestLang(lang = language.en) {
	//เปลี่ยนภาษาหน้า request
	...
	const formPersonalinfoFirstName =
		formPersonalInfo.childNodes[3].childNodes[3].childNodes[1];
	const formPersonalinfoLastname =
		formPersonalInfo.childNodes[3].childNodes[5].childNodes[1];
	const formStudentId =
		formPersonalInfo.childNodes[5].childNodes[5].children[0];
	const formPersonalInfoPhone =
		formPersonalInfo.childNodes[7].childNodes[1].children[0];
	...
	for (let i = 0; i < formSelect[0].length; i++) {
		// เปลี่ยนภาษาในส่วนเลือกคำนำหน้า
	...
	for (let i = 0; i < formSelect[2].length; i++) {
		// เปลี่ยนภาษาในส่วนเลือกคณะ
		formSelect[2][i].textContent = Object.values(
			langF.personalInfo.faculty.chocice,
		)[i];
	}
	...
	const formLocationSubdistrict =
		formLocation.children[1].children[2].children[0];
...
function changeUserProfileLang(lang = language.en) {
	...
	for (let i = 0; i < userMenu.children.length - 1; i++) {
		userMenu.children[i].children[0].textContent = Object.values(
			langU.personalInfo,
		)[i];
	}
