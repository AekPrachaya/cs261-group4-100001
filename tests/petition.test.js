...
describe("Petition Handlers API", () => {
    ...
    it("should get petitions by role and status", async () => {
        const petition = await createPetition();

        petition.status = "approved";
        await request(app).put(`/api/petition/${petition.id}`).send(petition);

        const res = await request(app)
            .get("/api/petitions/role/staff?status=approved")
            .send();
